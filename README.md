...
仓库中包含两个 Skill 入口：根目录 Skill 面向日常检索、引用和回答；`skills/nihaisha-rag-builder` 面向维护者构建、增量更新、FAISS 重建和发布校验。两者都属于这个仓库，只是职责分开。

仓库还包含一个精简的 Python 模块和一份已经构建好的本地数据库。Python 模块同时提供正式 CLI，供两个 Skill 分别调用成熟的使用逻辑和构建逻辑。
...
维护者需要重建数据库或增量更新资料时，应使用 `skills/nihaisha-rag-builder`，并参考 `docs/BUILD_AND_UPDATE.md`；README 只保留用户视角说明，不承载具体构建步骤。
