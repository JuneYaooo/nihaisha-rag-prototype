...
Answer from bundled course PDFs. Authoritative runtime evidence is an original PDF paragraph with portable filename, page, and excerpt; retrieval aids are navigation only. Run in this Skill directory; DB: `data/pdf_rag_bge_m3/rag.sqlite`.
...
1. Cite only retrieved `course_primary` PDF originals. If evidence is insufficient, say so; never fill from memory.
2. `derived` triples, guide nodes, expansions, flows, and follow-ups only navigate to originals; never cite them standalone. Follow-ups must arise from the query and retrieved evidence, not a fixed checklist.
3. This runtime has no external/classical retrieval. Never reconstruct absent ancient quotations from model memory.
4. `classic_primary` and `reference_secondary` are future versioned layers. User-supplied or explicitly authorized outside research must be labeled “external / not retrieved from bundled DB,” verified and cited separately, without bundled authority.
5. Cite lecture claims and classical originals separately; never turn a paraphrase into an ancient quote.
